#!/usr/bin/env python3
# Apply Scout Research Engine v2 Step 10:
# Orchestrator integration test + CI wiring.
#
...
GITHUB_WORKFLOW_YML = r'''
name: Knowledge package checks
...
jobs:
  knowledge:
    ...
    steps:
      ...
      - name: Typecheck knowledge package
        run: npm run typecheck:knowledge

      - name: Test knowledge package
        run: npm run test:knowledge
'''


def update_root_package_json() -> None:
    path = ROOT / "package.json"
    data = json.loads(path.read_text(encoding="utf-8"))

    scripts = data.setdefault("scripts", {})
    scripts["typecheck:knowledge"] = "npm --workspace packages/knowledge run typecheck"
    scripts["test:knowledge"] = "npm --workspace packages/knowledge test"
...
'''
    if "Done in v2 Slice 9" not in text:
        text = text.rstrip() + "\n\n" + append.strip() + "\n"
...
'''
    if "Research Engine v2 Slice 9" not in text:
        text = text.rstrip() + "\n\n" + append.strip() + "\n"
    path.write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    write(
        "packages/knowledge/src/research/__tests__/research-orchestrator.test.ts",
        ORCHESTRATOR_TEST_TS,
    )
    write(".github/workflows/knowledge-tests.yml", GITHUB_WORKFLOW_YML)
    ...
    print("\nIf this passes, run the real web-research smoke test once.")
    print("Next feature after green tests: source freshness + diversity scoring, not GraphAgent yet.")
